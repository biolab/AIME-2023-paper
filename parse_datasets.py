...
TCGA_DATASETS = [
    ...
    "THCA",
    "UCEC",
]  # datasets with more than 100 samples


def load_cancer_type():

    cancer = pd.read_csv(
        "data/GSE62944_06_01_15_TCGA_24_CancerType_Samples.txt.gz",
        sep="\t",
        header=None,
        index_col=[0],
    )
...
def load_RNA_seq_expressions():
    return pd.read_csv(
        "data/GSM1536837_06_01_15_TCGA_24.tumor_Rsubread_TPM.txt.gz", sep="\t"
    )


def load_clinical_metadata():
    return pd.read_csv(
        "data/GSE62944_06_01_15_TCGA_24_548_Clinical_Variables_9264_Samples.txt.gz",
        sep="\t",
        index_col=[0],
        low_memory=False,
        dtype=object,
...
def parse_RNA_seq_data():
    ...
    print("Loading the expression file (ETA 2 minutes)")
    cancer = load_cancer_type()
    expressions = load_RNA_seq_expressions()

    print("Extracting individual TCGA datasets. (ETA 5 minutes)")
